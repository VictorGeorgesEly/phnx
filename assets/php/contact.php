...
//
  $email = 'phnx@juniorisep.com'; // Your email address
  $name = 'PHNX'; // Your name
  $subject = 'PHNX - Message du site !'; // Subject line
  $body = '
  <html>
    <head>
      <title>' . $subject . '</title>
    ...
    <body>
      <p><strong style="width: 80px;">Nom: </strong>' . $_POST['name'] . '</p>
      <p><strong style="width: 80px;">Email: </strong>' . $_POST['email'] . '</p>
      <p><strong style="width: 80px;">Message: </strong>' . nl2br($_POST['message']) . '</p>
    </body>
  </html>
  ';
//
...
//
  ...
  if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    try {
      ...
      $mail->send();
      throw new Exception('Votre message a bien été envoyé !', 0); // success message
