  ...
  function fn_saveToFile($form_email) {
    if (defined('FILE_PATH')) {
      if (strpos(file_get_contents(FILE_PATH), $form_email) == true) {
        throw new Exception('Vous êtes déjà inscrit', 1); // error message
      } else {
        file_put_contents(FILE_PATH, date("Y-m-d H:i:s").' - '.$form_email."\n", FILE_APPEND | LOCK_EX); // LOCK_EX available since PHP 5.1.0+
        throw new Exception('Merci ! Vous obtiendrez les informations en avant-première !', 0); // success message
      }
    } else {
      throw new Exception('Une erreur est survenue, merci de réessayer plus tard.', 1); // error message
    }
  }
